...
VERSION=`head Makefile | grep -Po 'VERSION = \K.*'|tr -d '\n\r'`
PATCHLEVEL=`head Makefile | grep -Po 'PATCHLEVEL = \K.*'|tr -d '\n\r'`
SUBLEVEL=`head Makefile | grep -Po 'SUBLEVEL = \K.*'|tr -d '\n\r'`

telegram-send -s sticker.webp
...
if [ $? -eq 0 ]; then
	...
	telegram-send $TIME_CURR" - Make "$VERSION"."$PATCHLEVEL"."$SUBLEVEL" defconfig successful"
else
	TIME_CURR=$(TZ=":Asia/Kolkata" date +"%d.%m.%Y-%T")
	telegram-send $TIME_CURR" - Error in make "$VERSION"."$PATCHLEVEL"."$SUBLEVEL" defconfig building"
fi
...
if [ $? -eq 0 ]; then
	...
	TIME_CURR=$(TZ=":Asia/Kolkata" date +"%d.%m.%Y-%T")
	telegram-send $TIME_CURR" - Error in building "$VERSION"."$PATCHLEVEL"."$SUBLEVEL
	telegram-send -f build.log
